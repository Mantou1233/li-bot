...
async function Sign(json, client_info = null) {
	return new Promise((resolve, reject) => {
		...
		let msg_seq = parseInt(`${time}${random_(100, 999)}`);
		...
		let get_json = async function () {
			let ChatHistory = await Bot.pickFriend(Bot.uin).getChatHistory(
				0,
				20
			);
			ChatHistory.reverse();
			...
			result.message = new Promise((resolve, reject) => {
				...
				let get_message = async function (status = false) {
					...
					for (let i = 0; i < 3; i++) {
						let ChatHistory =
							send_type == 1
								? await Bot.pickGroup(recv_uin).getChatHistory(
										seq,
										20
								  )
								: await Bot.pickFriend(recv_uin).getChatHistory(
										seq,
										20
								  );
						if (send_type == 1) {
