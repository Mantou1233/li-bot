...
async function load(client: Client, cm: CommandManager) {
		...
		handler: async msg => {
			...
			if (args[1] == "全部" || args[1] == "all") {
				if (p.bank !== 0) {
					...
					p.save();
					return;
				} else return msg.reply("你的银行里没钱！", true);
			} else {
				const value = +args[1];
				if (value % 1 !== 0)
					return msg.reply("存的款项必须是数字！", true);
				else if (value < 0) return msg.reply("草 不要拿负数！", true);
				else if (value > p.bank)
					return msg.reply(
						"你的银行没有这么多钱！如果你还是想取的话请用 取款 全部(或all)!",
						true
					);
				else {
					p.coin += value;
	...
	cm.register({
		...
		handler: async msg => {
			let args = ap(msg.content);
			let p = await UserProfile(msg);
			let tp = await UserProfile(+args[1]);

			const ok = await tp.check();
			console.log(ok, tp.__prefix + args[1]);
			if (!ok) return msg.reply("对方没有账号！", true);
			if (args.length < 3) return msg.reply("请提供金额和用户！", true);
			const value = +args[2];
			if (isNaN(value) || value < 100)
				return msg.reply(
					"请提供合法的金额！转帐需要至少转100元！",
					true
				);
			if (p.coin < value + Math.floor(value * 0.01))
				return msg.reply(
					`你没有足够的金钱！ (转帐收取1%手续费，请确认你有没有足够的金额！)`,
					true
				);
			tp.coin += value;
			...
			tp.save();
			msg.reply(
				`成功转账给qid${args[1]}的用户！扣去1%手续费(${Math.floor(
					value * 0.01
				)}金币)，你现在有${p.coin}元。`,
				true
			);
